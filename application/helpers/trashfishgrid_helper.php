<?php
     ...
     * Parse date
    **/
    function trashfish_parse_date($date, $format = 'Y-m-d')
    {
        $pformat = preg_replace('/([dDljmMFnYyGgHhAais])/', '%$1', $format);

        $ret = trashfish_strptime($date, $pformat);
     ...
     * Format date
    **/
    function trashfish_format_date(
            $date,
            $input_format,
            $output_format,
        ...
        )
    {
        if (!is_array($date))
        {
            ...
            $date = trashfish_strptime($date, $pformat);
